...
func (ws *WebSSH) AddWebsocket(id string, conn *websocket.Conn, username string, password string) {
	...
	go func() {
		ws.logger.Printf("%s server exit %v", id, ws.server(value, username, password))
	}()
}

// AddSSHConn add ssh netword connect
func (ws *WebSSH) AddSSHConn(id string, conn net.Conn, username string, password string) {
...
func (ws *WebSSH) server(value *storeValue, username string, paassword string) error {
	...
	for {
		...
		case messageTypeLogin:
			...
			config.Auth = append(config.Auth, ssh.Password(paassword))
